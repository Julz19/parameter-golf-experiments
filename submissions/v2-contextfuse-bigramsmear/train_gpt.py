# Standalone record-folder training script for ContextFuse-2048-BigramSmear.
...
class Hyperparameters:
    ...
    mixed_int6_export = bool(int(os.environ.get("MIXED_INT6_EXPORT", "0")))
    int6_categories = tuple(
        category for category in os.environ.get("INT6_CATEGORIES", "mlp,attn").split(",") if category
    )
...
def eval_val_sliding(
    ...
    if total_tokens <= seq_len:
        ...
        tail_start = total_tokens - seq_len
        if window_starts[-1] != tail_start:
    ...
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            for i, ws in enumerate(batch_ws):
                window_idx = my_s + bi + i
                wlen = wlens[i]
                prev_end = 0 if window_idx == 0 else min(window_starts[window_idx - 1] + seq_len, total_tokens)
                score_start = max(prev_end - ws, 0)
                if score_start >= wlen:
                    continue
...
def should_keep_fp16_tensor(name: str, state_dict: dict[str, Tensor]) -> bool:
    ...
    if FP16_LATE_K_LAYERS > 0 and name.endswith("c_k.weight"):
        ...
        return any(f"blocks.{i}." in name for i in range(late_start, num_layers_total))
    return False
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        if should_keep_fp16_tensor(name, state_dict):
            passthrough_orig_dtypes[name] = str(t.dtype).removeprefix("torch.")
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.c_q._ortho_init = True
        self.c_k._ortho_init = True
        self.c_v._ortho_init = True
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        for module in self.modules():
            ...
            elif ORTHO_INIT and isinstance(module, nn.Linear) and getattr(module, "_ortho_init", False):
                nn.init.orthogonal_(module.weight, gain=1.0)

    def _encode_text_tokens(self, input_ids: Tensor) -> Tensor:
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        x = self._encode_text_tokens(input_ids)
...
def main() -> None:
    ...
    log0(f"ortho_init:{ORTHO_INIT}")
    log0(
        f"use_smear_gate:{args.use_smear_gate} "
        f"bigram_vocab_size:{args.bigram_vocab_size} bigram_dim:{args.bigram_dim} "
        f"mixed_int6_export:{args.mixed_int6_export} int6_categories:{','.join(args.int6_categories) or '-'}"
    )
    log0(
        f"swa_enabled:{args.swa_enabled} "
        f"swa_start_frac:{args.swa_start_frac:.3f} swa_every:{args.swa_every}"
    )
    ...
    while True:
        ...
        if args.swa_enabled and scale < args.swa_start_frac and step % args.swa_every == 0:
            ...
            if swa_state is None:
                swa_state = {
                    name: (tensor.detach().cpu().float() if tensor.is_floating_point() else tensor.detach().cpu().clone())
                ...
                for name, tensor in current_state.items():
                    if tensor.is_floating_point():
                        swa_state[name] += tensor.detach().cpu().float()
                    else:
    ...
    if args.swa_enabled and swa_state is not None and swa_count > 1:
        ...
        avg_state = {}
        for name, tensor in current_state.items():
            avg_tensor = swa_state[name]
            avg_state[name] = (avg_tensor / swa_count).to(dtype=tensor.dtype) if tensor.is_floating_point() else avg_tensor
    ...
    if args.mixed_int6_export:
        sd_cpu = {k: v.detach().cpu() for k, v in base_model.state_dict().items()}
        quant_result, quant_meta = mixed_quantize_int6(sd_cpu, set(args.int6_categories))
        quant_buf = io.BytesIO()
        torch.save({"w": quant_result, "m": quant_meta}, quant_buf)
        quant_raw = quant_buf.getvalue()
        compressor = "zstd" if zstd is not None else "zlib"
        quant_blob = (
        ...
        if master_process:
            ...
            code_bytes = len(code.encode("utf-8"))
            log0(f"Serialized model int6+{compressor}: {quant_file_bytes} bytes")
            log0(f"Total submission size int6+{compressor}: {quant_file_bytes + code_bytes} bytes")
        ...
        base_model.load_state_dict(dequantize_mixed_int6(quant_state["w"], quant_state["m"], sd_cpu), strict=True)
        torch.cuda.synchronize()
        ...
        torch.cuda.synchronize()
        log0(
            f"final_int6_{compressor}_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
        )
        log0(f"final_int6_{compressor}_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
    else:
