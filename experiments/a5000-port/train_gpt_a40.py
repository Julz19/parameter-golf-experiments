...
class Hyperparameters:
    ...
    final_averaging = os.environ.get("FINAL_AVERAGING", "ema").strip().lower()
    ...
    ttt_score_batch_seqs = int(os.environ.get("TTT_SCORE_BATCH_SEQS", os.environ.get("EVAL_BATCH_SEQS", 32)))
...
class DistributedTokenLoader:
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        ...
        if local_tokens < seq_len or local_tokens % seq_len != 0:
            raise ValueError(
                "TRAIN_BATCH_TOKENS must yield an integer number of full sequences per rank and micro-step; "
...
def main() -> None:
    ...
    if requested_grad_accum_steps > 0:
        ...
        grad_accum_source = "env"
    else:
        if 8 % world_size != 0:
            raise ValueError(f"WORLD_SIZE={world_size} must divide 8 so grad_accum_steps stays integral")
        grad_accum_steps = 8 // world_size
        grad_accum_source = "derived_8_over_world_size"
    ...
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps} grad_accum_source:{grad_accum_source}")
    ...
    log0(
        ...
        f"swa_enabled:{args.swa_enabled} swa_every:{args.swa_every} "
        f"final_averaging:{args.final_averaging}"
    )
    ...
    if args.final_averaging == "raw":
        log0("final_averaging:keeping raw stop-time weights")
    elif args.final_averaging == "swa":
        if not args.swa_enabled or swa_state is None or swa_count <= 0:
            raise RuntimeError(
                "FINAL_AVERAGING=swa requested, but no SWA snapshots are available. "
                "Set SWA_ENABLED=1 and ensure the run reaches at least one SWA capture."
            )
    ...
    elif args.final_averaging == "lawa":
        if not args.lawa_enabled or len(lawa_queue) <= 1:
            raise RuntimeError(
                "FINAL_AVERAGING=lawa requested, but no LAWA snapshots are available. "
                "Set LAWA_ENABLED=1 and ensure the run reaches multiple LAWA captures."
            )
        ...
        base_model.load_state_dict(avg_state, strict=True)
    elif args.final_averaging == "ema":
        if ema_state is None:
            raise RuntimeError("FINAL_AVERAGING=ema requested, but EMA is disabled.")
        ...
        raise ValueError(
            f"Unsupported FINAL_AVERAGING={args.final_averaging!r}; "
            "expected one of: raw, ema, swa, lawa"
        )
    torch.cuda.synchronize()
    ...
    if args.ttt_enabled:
        ...
        ttt_loss, ttt_bpb = eval_val_sliding_ttt(
            args, eval_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            stride=args.eval_stride, batch_seqs=args.ttt_score_batch_seqs, log0=log0,
        )
        torch.cuda.synchronize()
